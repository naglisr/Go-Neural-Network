...
class Group(object):
    ...
    def is_adjacent(self, other):
        """Check if two groups have diagonally adjacent points
            For use in calculating territory
        """
        for s1 in self.stones:
            for s2 in other.stones:
                if abs(s1.point.x-s2.point.x) == 1 and abs(s1.point.y-s2.point.y) == 1:
                    return True
        return False
...
class Board(object):
    ...
    def territory(self, color):
        pass
        # super_groups = []
        # iterable_groups = iter(self.groups)
        # group_pairs = zip(iterable_groups, iterable_groups)
        # graph = utils.graph.Graph(len(self.groups))
        #
        # for g1, g2 in group_pairs:
        #     if g1.is_adjacent(g2):
        #         graph.addEdge(g1, g2)
        #
        # cycles = graph.isCyclic()
